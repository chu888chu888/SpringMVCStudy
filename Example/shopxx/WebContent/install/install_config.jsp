...
<%
	...
	if (!failed) {
		...
		logoElement.attribute("value").setValue(base + "/upload/image/logo.gif");
	...
	if (!failed) {
		try {
			FileUtils.writeStringToFile(installLockConfigFile, "SHOP++ INSTALL LOCK - SHOPXX.NET");
		} catch (IOException e) {
			failed = true;
	...
	if (!failed) {
		try {
			FileUtils.writeStringToFile(installInitConfigFile, "SHOP++ INSTALL INIT - SHOPXX.NET");
			FileUtils.deleteQuietly(indexJspFile);
		} catch (IOException e) {
			failed = true;
